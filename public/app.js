...
let adminData=null, adminGroupsData=[], adminRankings=[], adminWeekly=null, adminPeriods=[], adminDaily=[], adminScoreMap={}, adminSection='data', adminManageSection=null;
...
async function openArea(a){playSound('nav');area=a;$('homeTab')?.classList.remove('active');$('admin').classList.add('hidden');$('reporting').classList.remove('hidden');$('classTab').classList.toggle('active',a==='class');$('dormTab').classList.toggle('active',a==='dorm');$('adminTab').classList.remove('active');const [loadedGroups,leader]=await Promise.all([api('/api/groups?type='+a),api('/api/leaderboard').catch(()=>null)]);groups=loadedGroups;leadingGroupName=leader?.name||leadingGroupName;currentGroup=null;renderGroupPicker()}
function renderGroupPicker(){$('reporting').innerHTML=`<div class="panel reporting-groups-panel"><h3>${area==='class'?'בחר קבוצת לימודים':'בחר קבוצת פנימייה'}</h3><div class="reporting-group-list">${groups.map(g=>`<details class="admin-collapsible reporting-group-accordion ${g.name===leadingGroupName?'leader-picker':''}" data-report-group="${g.id}"><summary><span class="summary-icon">${g.name===leadingGroupName?'🏆':'🏫'}</span><span class="grow"><b>${esc(g.name)}</b></span><span class="summary-arrow" aria-hidden="true"></span></summary><div id="reportGroup_${g.id}" class="collapsible-body reporting-group-students"></div></details>`).join('')||'<div class="small">אין קבוצות שהוקצו למשתמש זה.</div>'}</div></div>`}
async function selectGroup(id,details){playSound('nav');currentGroup=id;await loadStudents(details)}
async function loadStudents(details=null){if(!currentGroup)return;const rows=await api('/api/groups/'+currentGroup+'/students');const host=$('reportGroup_'+currentGroup)||$('students');if(host)host.innerHTML=reportStudentsHtml(rows,groups.find(g=>g.id===currentGroup)?.name||'');startCooldownClock()}
...
function manageSectionBody(){return adminManageSection==='staff'?adminUsers(adminData.users,adminGroupsData):adminManageSection==='score'?adminScoreManagement():adminManageSection==='engagement'?adminEngagement():adminManageSection==='rules'?adminRulesEditor():adminManageSection==='students'?`${adminStudents(adminData.students)}${adminGroups(adminGroupsData.filter(g=>g.type==='class'),adminData.students)}`:''}
...
function setManageSection(s){playSound('nav');const closing=adminManageSection===s;adminManageSection=closing?null:s;renderAdminContent();if(closing)return;if(s==='rules')loadCampaignRulesEditor();if(s==='engagement')loadEngagementData();if(s==='score')setTimeout(()=>searchScoreStudents(false),0);requestAnimationFrame(()=>requestAnimationFrame(()=>{const card=document.querySelector('.manage-card.active');if(card)revealOpenedItem(card)}))}
...
async function rulesInsertLink(){
  const url=await appPrompt('הדבק כתובת קישור:',{title:'הוספת קישור',icon:'🔗',placeholder:'https://...'});
  if(!url)return;
...
async function doScoreAdjustFor(btn,sign){
  ...
  if(!await appConfirm(`${delta>0?'להוסיף':'להפחית'} ${n} נקודות ל${name}?`,{title:'אישור שינוי ניקוד',icon:'🎯'}))return;
  const management_password=await appPrompt('הזן סיסמת מנהל לאישור הפעולה:',{title:'אימות מנהל',icon:'🔐',password:true});if(!management_password)return;
...
async function resetStudentScoreFor(btn){
  ...
  if(!await appConfirm(`לאפס את הניקוד של ${name} (${current} נק׳) ל-0?`,{title:'איפוס ניקוד',icon:'↺'}))return;
  const management_password=await appPrompt('הזן סיסמת מנהל לאישור האיפוס:',{title:'אימות מנהל',icon:'🔐',password:true});if(!management_password)return;
...
async function doScoreAdjust(sign){const n=Math.abs(Math.trunc(Number($('scoreDelta')?.value)));if(!n)return alert('יש להזין מספר נקודות');const delta=n*sign,reason=$('scoreReason')?.value.trim()||'';if(!await appConfirm(`${delta>0?'להוסיף':'להפחית'} ${n} נקודות ל${scoreSelectedStudent.name}?`,{title:'אישור שינוי ניקוד',icon:'🎯'}))return;const management_password=await appPrompt('הזן סיסמת מנהל לאישור הפעולה:',{title:'אימות מנהל',icon:'🔐',password:true});if(!management_password)return;try{const r=await api('/api/admin/score-management/'+scoreSelectedStudent.id+'/adjust',{method:'POST',body:JSON.stringify({mode:'adjust',delta,reason,management_password})});scoreSelectedStudent.score=r.after;await renderScoreStudentCard();await Promise.all([loadAdminAll(),loadLeader()]);alert(`הניקוד עודכן ל-${r.after}`)}catch(e){alert(e.message)}}
async function resetStudentScore(){if(!scoreSelectedStudent||scoreSelectedStudent.score===null)return alert('הניקוד הנוכחי עדיין נטען. נסה שוב בעוד רגע.');if(!await appConfirm(`לאפס את הניקוד של ${scoreSelectedStudent.name} מ-${scoreSelectedStudent.score} ל-0?\nהדיווחים וההיסטוריה יישמרו.`,{title:'איפוס ניקוד',icon:'↺'}))return;const management_password=await appPrompt('הזן סיסמת מנהל לאישור איפוס:',{title:'אימות מנהל',icon:'🔐',password:true});if(!management_password)return;const reason=(await appPrompt('סיבת האיפוס (אופציונלי):',{title:'סיבת האיפוס',icon:'📝',optional:true}))||'';try{const r=await api('/api/admin/score-management/'+scoreSelectedStudent.id+'/adjust',{method:'POST',body:JSON.stringify({mode:'reset',reason,management_password})});scoreSelectedStudent.score=r.after;await renderScoreStudentCard();await Promise.all([loadAdminAll(),loadLeader()]);alert('הניקוד אופס. ההיסטוריה נשמרה.')}catch(e){alert(e.message)}}
async function undoLastScoreChange(){if(!scoreSelectedStudent||!await appConfirm('לבטל את השינוי הידני האחרון?',{title:'ביטול שינוי',icon:'↶'}))return;const management_password=await appPrompt('הזן סיסמת מנהל:',{title:'אימות מנהל',icon:'🔐',password:true});if(!management_password)return;try{await api('/api/admin/score-management/'+scoreSelectedStudent.id+'/undo-last',{method:'POST',body:JSON.stringify({management_password})});const rows=await api('/api/admin/score-management/students?q='+encodeURIComponent(scoreSelectedStudent.name));const fresh=rows.find(x=>Number(x.id)===Number(scoreSelectedStudent.id));if(fresh)scoreSelectedStudent.score=fresh.score;await renderScoreStudentCard();await Promise.all([loadAdminAll(),loadLeader()])}catch(e){alert(e.message)}}
...
async function startNewPeriod(){const name=$('newPeriodName').value.trim(),started_at=$('newPeriodDate')?.value;if(!name)return alert('הזן שם לתקופה');if(!await appConfirm(`לפתוח תקופת ניקוד חדשה בשם "${name}"? התקופה הנוכחית תיסגר וכל ההיסטוריה תישמר.`,{title:'תקופת ניקוד חדשה',icon:'🗓️'}))return;const management_password=await appPrompt('הזן סיסמת מנהל לאישור פתיחת תקופה חדשה:',{title:'אימות מנהל',icon:'🔐',password:true});if(!management_password)return;await api('/api/admin/periods/start',{method:'POST',body:JSON.stringify({name,started_at,management_password})});await Promise.all([refreshAdminEdit(),loadLeader()]);alert('התקופה החדשה נפתחה')}
async function renamePeriod(id,current){const name=(await appPrompt('שם חדש לתקופה:',{title:'שינוי שם תקופה',icon:'✏️',value:current}))||'';if(!name.trim())return;const management_password=await appPrompt('הזן סיסמת מנהל:',{title:'אימות מנהל',icon:'🔐',password:true});if(!management_password)return;await api('/api/admin/periods/'+id+'/name',{method:'PUT',body:JSON.stringify({name:name.trim(),management_password})});await refreshAdminEdit()}
...
async function deleteStudent(id){if(await appConfirm('למחוק תלמיד?',{title:'מחיקת תלמיד',icon:'🗑️',danger:true})){await api('/api/admin/students/'+id,{method:'DELETE'});refreshAdminEdit()}}
...
async function deleteGroup(id){if(await appConfirm('למחוק את הקבוצה גם מלימודים וגם מהפנימייה?',{title:'מחיקת קבוצה',icon:'🗑️',danger:true})){await api('/api/admin/groups/'+id,{method:'DELETE'});refreshAdminEdit()}}
...
async function addUser(){
  ...
  if(role==='admin'){
    management_password=await appPrompt('כדי להוסיף משתמש לצוות ניהול יש להזין את סיסמת הניהול הראשית:',{title:'אימות מנהל',icon:'🔐',password:true});
...
async function saveUser(id){
  ...
  if(role==='admin'&&existing?.role!=='admin'){
    management_password=await appPrompt('כדי להעביר משתמש לצוות ניהול יש להזין את סיסמת הניהול הראשית:',{title:'אימות מנהל',icon:'🔐',password:true});
    if(!management_password)return;
  }
  try{
...
async function saveReminder(id){
  ...
  appToast(off?'התזכורת בוטלה':'התזכורת נשמרה');
}
async function sendReminderNow(id){if(!await appConfirm('לשלוח עכשיו תזכורת לאיש הצוות לפי הערוצים שנבחרו?',{title:'שליחת תזכורת',icon:'🔔'}))return;try{const r=await api('/api/admin/users/'+id+'/reminder/send-now',{method:'POST'});alert(`הבקשה התקבלה לשליחה. אימייל: ${r.emailSent||0}, Push: ${r.pushSent||0}, WhatsApp: ${r.whatsappSent||0}. שים לב: אישור השליחה אינו אישור מסירה לטלפון.`)}catch(e){alert(e.message)}}
...
async function deleteUser(id){if(await appConfirm('למחוק את משתמש הצוות?',{title:'מחיקת משתמש',icon:'🗑️',danger:true})){await api('/api/admin/users/'+id,{method:'DELETE'});refreshAdminEdit()}}
...
function appToast(message,opts={}){
  const host=document.getElementById('appToastHost')||(()=>{const h=document.createElement('div');h.id='appToastHost';h.className='app-toast-host';document.body.appendChild(h);return h})();
  const t=document.createElement('div');t.className='app-toast '+(opts.error?'is-error':'is-success');
  t.innerHTML=`<span class="app-toast-icon">${opts.error?'!':'✓'}</span><span>${esc(String(message||''))}</span>`;host.appendChild(t);
  requestAnimationFrame(()=>t.classList.add('show'));setTimeout(()=>{t.classList.remove('show');setTimeout(()=>t.remove(),220)},opts.duration||2000);
}
window.alert=(message)=>appToast(message,{error:/שגיאה|נכשל|לא ניתן|חובה|יש להזין|יש לאשר/.test(String(message||''))});
function appDialog({title='אישור פעולה',message='',icon='✓',confirmText='אישור',cancelText='ביטול',input=false,password=false,value='',placeholder='',danger=false,optional=false}={}){
  return new Promise(resolve=>{
    document.getElementById('appActionDialog')?.remove();const back=document.createElement('div');back.id='appActionDialog';back.className='app-dialog-backdrop';
    back.innerHTML=`<div class="app-dialog-card" role="dialog" aria-modal="true"><div class="app-dialog-icon">${esc(icon)}</div><h3>${esc(title)}</h3><div class="app-dialog-message">${esc(message).replace(/\n/g,'<br>')}</div>${input?`<div class="app-dialog-input-wrap"><input class="input app-dialog-input" type="${password?'password':'text'}" value="${escAttr(value)}" placeholder="${escAttr(placeholder)}" autocomplete="${password?'current-password':'off'}">${password?'<button type="button" class="app-dialog-eye" aria-label="הצג סיסמה">👁️</button>':''}</div>`:''}<div class="app-dialog-actions"><button class="btn app-dialog-cancel">${esc(cancelText)}</button><button class="btn ${danger?'danger':'active'} app-dialog-confirm">${esc(confirmText)}</button></div></div>`;
    document.body.appendChild(back);const field=back.querySelector('.app-dialog-input');const finish=v=>{back.classList.remove('show');setTimeout(()=>back.remove(),180);resolve(v)};
    back.querySelector('.app-dialog-cancel').onclick=()=>finish(input?null:false);back.querySelector('.app-dialog-confirm').onclick=()=>{if(input){const v=field.value;if(!optional&&!v.trim()){field.focus();field.classList.add('input-error');return}finish(v)}else finish(true)};
    back.querySelector('.app-dialog-eye')?.addEventListener('click',e=>{field.type=field.type==='password'?'text':'password';e.currentTarget.textContent=field.type==='password'?'👁️':'🙈'});
    back.addEventListener('click',e=>{if(e.target===back)finish(input?null:false)});field?.addEventListener('keydown',e=>{if(e.key==='Enter')back.querySelector('.app-dialog-confirm').click();if(e.key==='Escape')back.querySelector('.app-dialog-cancel').click()});
    requestAnimationFrame(()=>{back.classList.add('show');field?.focus()});
  })
}
function appConfirm(message,opts={}){return appDialog({...opts,message,confirmText:opts.confirmText||'אישור'})}
function appPrompt(message,opts={}){return appDialog({...opts,message,input:true,confirmText:opts.confirmText||'אישור'})}

// Accordion behavior: within the same visual group, opening one item closes its siblings.
document.addEventListener('toggle',e=>{const d=e.target;if(!(d instanceof HTMLDetailsElement)||!d.open)return;const parent=d.parentElement;if(!parent)return;[...parent.children].forEach(x=>{if(x!==d&&x instanceof HTMLDetailsElement&&x.open)x.open=false})},true);
...
async function deleteChallenge(id){if(!await appConfirm('למחוק את האתגר?',{title:'מחיקת אתגר',icon:'🗑️',danger:true}))return;try{await api('/api/admin/challenges/'+id,{method:'DELETE'});await loadEngagementData()}catch(e){alert(e.message)}}
...
/* V2.5.13 — accordion placement + nested admin accordions
...
function revealOpenedItem(target){
  ...
  requestAnimationFrame(()=>requestAnimationFrame(()=>{
    ...
    const gap=10;
    const y=Math.max(0,window.scrollY+r.top-stickyHeaderBottom()-gap);
    window.scrollTo({top:y,behavior:'smooth'});
  }));
}
function accordionFamily(d){
  if(d.classList.contains('reporting-group-accordion'))return 'reporting-group-accordion';
  ...
  if(d.classList.contains('staff-user-accordion'))return 'staff-user-accordion';
...
document.addEventListener('toggle',e=>{
  ...
  if(d.classList.contains('reporting-group-accordion')){currentGroup=Number(d.dataset.reportGroup);loadStudents(d).catch(e=>alert(e.message))}
