...
function showInAppPush(m){let x=$('inAppPush');if(!x){x=document.createElement('div');x.id='inAppPush';x.className='inapp-push';document.body.appendChild(x)}x.innerHTML=`<button onclick="this.parentElement.classList.remove('show')">×</button><b>🔔 ${esc(m.title)}</b><div>${esc(m.body)}</div>`;x.classList.add('show');setTimeout(()=>x.classList.remove('show'),9000)}
