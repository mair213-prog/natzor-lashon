...
async function openScoreStudentInline(details,id,name,group){if(!details.open)return;document.querySelectorAll('.score-student-accordion[open]').forEach(d=>{if(d!==details)d.open=false});const el=details.querySelector('.score-inline-editor');if(el)el.innerHTML='<div class="small score-loading">טוען נתוני ניקוד...</div>';try{const x=await api('/api/admin/score-management/student/'+id);scoreSelectedStudent={id:x.id,name:x.name,group:group||x.group_name||'',score:Number(x.score||0)};await renderScoreStudentCard(el)}catch(e){if(el)el.innerHTML='<div class="small score-load-error">לא ניתן לטעון את נתוני הניקוד: '+esc(e.message)+'</div>'}}
