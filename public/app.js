...
let engagementChallenges=[], engagementAnnouncements=[], pushStats={total:0,study:0,dorm:0,admin:0}, pushInboxLastId=0, pushInboxTimer=null;
...
function showInAppPush(m){let x=$('inAppPush');if(!x){x=document.createElement('div');x.id='inAppPush';x.className='inapp-push';document.body.appendChild(x)}x.innerHTML=`<div class="inapp-push-backdrop"></div><div class="inapp-push-card" role="dialog" aria-modal="true" aria-label="התראה חדשה"><button class="inapp-push-close" onclick="document.getElementById('inAppPush').classList.remove('show')" aria-label="סגירת הודעה">×</button><div class="inapp-push-icon">🔔</div><div class="inapp-push-kicker">הודעה חדשה</div><h3>${esc(m.title)}</h3><div class="inapp-push-body">${esc(m.body)}</div><button class="btn primary inapp-push-ok" onclick="document.getElementById('inAppPush').classList.remove('show')">הבנתי</button></div>`;x.classList.add('show')}
async function pollPushInbox(){if(!me)return;try{const rows=await api('/api/push/inbox?after='+pushInboxLastId);for(const m of rows){pushInboxLastId=Math.max(pushInboxLastId,Number(m.id)||0);showInAppPush(m)}}catch{}}
async function startPushInbox(){if(pushInboxTimer)clearInterval(pushInboxTimer);try{const x=await api('/api/push/inbox?latest=1');pushInboxLastId=Number(x.latest)||0}catch{}pushInboxTimer=setInterval(pollPushInbox,12000)}
async function boot(){try{me=await api('/api/me');showApp();startPushInbox()}catch{}}
async function login(){try{me=await api('/api/auth/login',{method:'POST',body:JSON.stringify({email:$('email').value,password:$('password').value})});showApp();startPushInbox()}catch(e){$('loginMsg').textContent=e.message;$('loginMsg').classList.remove('hidden')}}
...
function reportStudentsHtml(rows,title,prefix=''){return `<div class="panel"><h2>${esc(title)}</h2><div class="small" style="margin-bottom:8px">השתתפות בשיעור: +3 נק׳, פעם אחת ביום בלימודים ופעם אחת בפנימייה. כל 20 השתתפויות מזכות בבונוס 5 נק׳. דיווח על שפה לא נקייה: −1 נק׳.</div>${rows.map(s=>studentCard(s,prefix)).join('')}</div>`}
...
function startCooldownClock(){if(cooldownTimer)clearInterval(cooldownTimer);const tick=()=>document.querySelectorAll('button[data-until]').forEach(btn=>{if(btn.dataset.daily==='1'){btn.disabled=true;btn.title='השתתפות בשיעור כבר דווחה היום באזור זה';return}const until=btn.dataset.until;if(!until){btn.title='';return}const ms=new Date(until).getTime()-Date.now();if(ms<=0){btn.disabled=false;btn.dataset.until='';btn.title='';return}const sec=Math.ceil(ms/1000),m=Math.floor(sec/60),r=String(sec%60).padStart(2,'0');btn.disabled=true;btn.title=`אפשר שוב בעוד ${m}:${r}`});tick();cooldownTimer=setInterval(tick,1000)}
...
function openReportsCenter(){
  modalShell('reportsModal','📋 מרכז דיווחים',`
    <div class="report-filters">
      ...
      <select id="rfType" class="input"><option value="">כל סוגי הדיווח</option><option value="plus">השתתפות בשיעור</option><option value="minus">דיווח על שפה לא נקייה</option></select>
...
function adminEngagement(){const staff=(adminData?.users||[]).filter(u=>u.active);return `<div class="panel"><h2>🚀 מעורבות ואתגרים</h2>
<h3>🎯 אתגר חדש</h3><div class="row"><input id="challengeTitle" class="input grow" placeholder="שם האתגר"><input id="challengeTarget" class="input" type="number" value="300" min="1"><button class="btn ok" onclick="createChallenge()">צור אתגר</button></div><input id="challengeDesc" class="input" placeholder="תיאור קצר"><div id="challengeList" class="engagement-list"><div class="small">טוען אתגרים...</div></div>
<hr><h3>📣 הודעה לצוות</h3><div class="row"><input id="announcementTitle" class="input grow" placeholder="כותרת"><button class="btn ok" onclick="createAnnouncement()">פרסם</button></div><textarea id="announcementBody" class="input" placeholder="תוכן ההודעה"></textarea><div class="row"><select id="announcementDuration" class="input" onchange="$('announcementExpires').classList.toggle('hidden',this.value!=='custom')"><option value="1d">הצג ליום</option><option value="3d">הצג ל-3 ימים</option><option value="7d" selected>הצג לשבוע</option><option value="30d">הצג לחודש</option><option value="none">ללא הגבלת זמן</option><option value="custom">תאריך ושעה מותאמים</option></select><input id="announcementExpires" class="input hidden" type="datetime-local"></div><div id="announcementList" class="engagement-list"><div class="small">טוען הודעות...</div></div>
<hr><h3>🔔 Push</h3><div class="small" id="pushStats">טוען נתוני מנויים...</div><div class="row"><input id="pushTitle" class="input" value="נצור לשונך"><input id="pushBody" class="input grow" placeholder="תוכן ההתראה"></div><div class="row"><select id="pushAudience" class="input" onchange="$('pushUser').classList.toggle('hidden',this.value!=='user')"><option value="all">כל המנויים</option><option value="study">צוות לימודים</option><option value="dorm">צוות פנימייה</option><option value="admin">מנהלים</option><option value="user">איש צוות מסוים</option></select><select id="pushUser" class="input hidden"><option value="">בחר איש צוות</option>${staff.map(u=>`<option value="${u.id}">${esc(u.name)} · ${u.role==='admin'?'מנהל':u.role==='study'?'לימודים':'פנימייה'}</option>`).join('')}</select><button class="btn" onclick="sendAdminPush()">שלח Push</button></div><div class="small">באפליקציה פתוחה ההתראה תוצג גם בתוך המערכת. כשהאפליקציה סגורה, Web Push תלוי בתמיכת המכשיר/ה-WebView.</div></div>`}
...
function renderEngagementLists(){
  ...
  if(cl){
    const items=engagementChallenges.map(c=>{
      const until=c.end_at?` · עד ${new Date(c.end_at).toLocaleString('he-IL')}`:'';
      return `<div class="manage-item"><div class="grow"><b>${esc(c.title)}</b><div class="small">יעד: ${c.target_points} נק׳ · ${c.active?'פעיל':'מושבת'}${until}</div><div class="small">${esc(c.description||'')}</div></div><div class="row"><button class="btn small-btn" onclick="editChallenge(${c.id})">✏️ עריכה</button><button class="btn small-btn" onclick="toggleChallenge(${c.id},${!c.active})">${c.active?'⏸️ השבת':'▶️ הפעל'}</button><button class="btn danger small-btn" onclick="deleteChallenge(${c.id})">🗑️ מחיקה</button></div></div>`;
    }).join('');
    cl.innerHTML=`<h4>אתגרים קיימים</h4>${items||'<div class="small">אין אתגרים</div>'}`;
  }
  const al=$('announcementList');
  if(al){
    const items=engagementAnnouncements.map(a=>{
      const until=a.expires_at?`מוצגת עד ${new Date(a.expires_at).toLocaleString('he-IL')}`:'ללא הגבלת זמן';
      return `<div class="manage-item"><div class="grow"><b>${esc(a.title)}</b><div class="small">${a.active?'פעילה':'מושבתת'} · ${until}</div><div>${esc(a.body)}</div></div><div class="row"><button class="btn small-btn" onclick="editAnnouncement(${a.id})">✏️ עריכה</button><button class="btn danger small-btn" onclick="deleteAnnouncement(${a.id})">🗑️ מחיקה</button></div></div>`;
    }).join('');
    al.innerHTML=`<h4>הודעות קיימות</h4>${items||'<div class="small">אין הודעות</div>'}`;
  }
  const ps=$('pushStats');if(ps)ps.textContent=`מכשירים רשומים: ${pushStats.total||0} · לימודים ${pushStats.study||0} · פנימייה ${pushStats.dorm||0} · מנהלים ${pushStats.admin||0}`;
}
async function createChallenge(){try{await api('/api/admin/challenges',{method:'POST',body:JSON.stringify({title:$('challengeTitle').value,description:$('challengeDesc').value,target_points:Number($('challengeTarget').value)})});$('challengeTitle').value='';$('challengeDesc').value='';await loadEngagementData();alert('האתגר נוצר 🎯')}catch(e){alert(e.message)}}
async function editChallenge(id){const c=engagementChallenges.find(x=>x.id===id);if(!c)return;const title=prompt('שם האתגר:',c.title);if(title===null)return;const desc=prompt('תיאור:',c.description||'');if(desc===null)return;const target=prompt('יעד נקודות:',c.target_points);if(target===null)return;const end=prompt('תאריך סיום בפורמט YYYY-MM-DD HH:MM (אפשר להשאיר ריק):',c.end_at?fmtDateTimeLocal(c.end_at).replace('T',' '):'');if(end===null)return;try{await api('/api/admin/challenges/'+id,{method:'PUT',body:JSON.stringify({title,description:desc,target_points:Number(target),end_at:end?new Date(end.replace(' ','T')).toISOString():null,active:c.active})});await loadEngagementData()}catch(e){alert(e.message)}}
...
async function editAnnouncement(id){const a=engagementAnnouncements.find(x=>x.id===id);if(!a)return;const title=prompt('כותרת:',a.title);if(title===null)return;const body=prompt('תוכן ההודעה:',a.body);if(body===null)return;const exp=prompt('מוצג עד (YYYY-MM-DD HH:MM), ריק = ללא הגבלה:',a.expires_at?fmtDateTimeLocal(a.expires_at).replace('T',' '):'');if(exp===null)return;try{await api('/api/admin/announcements/'+id,{method:'PUT',body:JSON.stringify({title,body,expires_at:exp?new Date(exp.replace(' ','T')).toISOString():null,active:a.active})});await loadEngagementData()}catch(e){alert(e.message)}}
