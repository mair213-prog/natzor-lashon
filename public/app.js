...
function adminManageView(){return `<div class="panel manage-switch"><h2>ניהול</h2><div class="small">בחר את תחום הניהול</div><div class="manage-tabs"><button class="manage-card ${adminManageSection==='students'?'active':''}" onclick="setManageSection('students')"><span class="manage-icon">🎓</span><b>תלמידים</b><small>תלמידים, קבוצות ותקופות ניקוד</small></button><button class="manage-card ${adminManageSection==='staff'?'active':''}" onclick="setManageSection('staff')"><span class="manage-icon">👥</span><b>צוות</b><small>משתמשים, הרשאות ותזכורות</small></button><button class="manage-card ${adminManageSection==='rules'?'active':''}" onclick="setManageSection('rules')"><span class="manage-icon">📜</span><b>כללי המבצע</b><small>עריכת המלל שמוצג לצוות</small></button></div></div><div id="manageBody">${adminManageSection==='staff'?adminUsers(adminData.users,adminGroupsData):adminManageSection==='rules'?adminRulesEditor():`${adminPeriodsView()}${adminStudents(adminData.students)}${adminGroups(adminGroupsData.filter(g=>g.type==='class'),adminData.students)}`}</div>`}
function setManageSection(s){playSound('nav');adminManageSection=s;renderAdminContent();if(s==='rules')loadCampaignRulesEditor()}
function campaignRulesHtml(text){return esc(text||'').split(/\n{2,}/).map(p=>`<p>${p.replace(/\n/g,'<br>')}</p>`).join('')}
async function openCampaignRules(){playSound('nav');try{const d=await api('/api/campaign-rules');modalShell('campaignRulesModal','📜 כללי המבצע',`<div class="campaign-rules-content">${campaignRulesHtml(d.text)}</div><div class="campaign-rules-sign">נצור לשונך <span>·</span> בוחרים לדבר נקי</div>`)}catch(e){alert(e.message)}}
function adminRulesEditor(){return `<div class="panel"><h2>📜 עריכת כללי המבצע</h2><div class="small" style="margin-bottom:10px">המלל שתשמור כאן יופיע בכפתור "כללי המבצע" לכל אנשי הצוות.</div><textarea id="campaignRulesEditor" class="input campaign-rules-editor" placeholder="טוען את כללי המבצע..."></textarea><div class="row"><button class="btn ok" onclick="saveCampaignRules()">💾 שמור כללי מבצע</button><button class="btn" onclick="loadCampaignRulesEditor()">↻ טען מחדש</button></div></div>`}
async function loadCampaignRulesEditor(){const el=$('campaignRulesEditor');if(!el)return;try{const d=await api('/api/campaign-rules');el.value=d.text||''}catch(e){el.value='';alert(e.message)}}
async function saveCampaignRules(){const el=$('campaignRulesEditor');if(!el)return;const text=el.value.trim();if(!text)return alert('כללי המבצע לא יכולים להיות ריקים');try{await api('/api/admin/campaign-rules',{method:'PUT',body:JSON.stringify({text})});alert('כללי המבצע נשמרו בהצלחה')}catch(e){alert(e.message)}}
