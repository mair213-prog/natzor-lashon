...
function scoreStudentRow(m,g){return `<details class="score-student-accordion" data-score-student="${m.id}" ontoggle="if(this.open)openScoreStudentInline(this,${m.id},'${escAttr(m.name)}','${escAttr(g||'')}')"><summary><span class="score-student-person">👤</span><span class="grow"><b>${esc(m.name)}</b></span><span class="modern-chevron" aria-hidden="true"><svg viewBox="0 0 24 24"><path d="M9 6l6 6-6 6"/></svg></span></summary><div class="score-inline-editor" id="scoreInline_${m.id}"><div class="small score-loading">טוען נתוני ניקוד...</div></div></details>`}
...
async function searchScoreStudents(){const el=$('scoreStudentResults');if(!el)return;const q=$('scoreStudentSearch')?.value||'';if(q.trim().length<1){el.innerHTML='';return}try{const rows=await api('/api/admin/score-management/students?q='+encodeURIComponent(q));el.innerHTML=rows.map(x=>`<details class="score-student-accordion search-score-row" ontoggle="if(this.open)openScoreStudentInline(this,${x.id},'${escAttr(x.name)}','${escAttr(x.group_name||'')}')"><summary><span class="score-student-person">👤</span><span class="grow"><b>${esc(x.name)}</b><small>${esc(x.group_name||'ללא קבוצת לימודים')}</small></span><strong>${Number(x.score||0)} נק׳</strong><span class="modern-chevron" aria-hidden="true"><svg viewBox="0 0 24 24"><path d="M9 6l6 6-6 6"/></svg></span></summary><div class="score-inline-editor" id="scoreInlineSearch_${x.id}"></div></details>`).join('')||'<div class="small">לא נמצאו תלמידים</div>'}catch(e){el.innerHTML='<div class="small">'+esc(e.message)+'</div>'}}
async function openScoreStudentInline(details,id,name,group){if(!details.open)return;document.querySelectorAll('.score-student-accordion[open]').forEach(d=>{if(d!==details)d.open=false});try{const rows=await api('/api/admin/score-management/students?q='+encodeURIComponent(name));const x=rows.find(r=>Number(r.id)===Number(id));if(!x)throw new Error('התלמיד לא נמצא');scoreSelectedStudent={id:x.id,name:x.name,group:group||x.group_name||'',score:Number(x.score||0)};const el=details.querySelector('.score-inline-editor');await renderScoreStudentCard(el)}catch(e){const el=details.querySelector('.score-inline-editor');if(el)el.innerHTML='<div class="small">'+esc(e.message)+'</div>'}}
async function renderScoreStudentCard(target){const x=scoreSelectedStudent,el=target||document.querySelector(`.score-student-accordion[open] .score-inline-editor`);if(!x||!el)return;let hist=[];try{hist=await api('/api/admin/score-management/'+x.id+'/history')}catch{}el.innerHTML=`<div class="score-editor-card inline"><div class="score-editor-head"><div><h3>${esc(x.name)}</h3><div class="small">${esc(x.group||'ללא קבוצה')}</div></div><div class="score-current"><span>ניקוד נוכחי</span><b>${Number(x.score||0)}</b></div></div><label class="small">מספר נקודות</label><input id="scoreDelta" class="input" type="number" step="1" placeholder="למשל 10"><label class="small">סיבת השינוי (מומלץ)</label><input id="scoreReason" class="input" maxlength="300" placeholder="למשל: תיקון דיווח / בונוס מיוחד"><div class="score-actions"><button class="btn ok" onclick="doScoreAdjust(1)">➕ הוסף נקודות</button><button class="btn danger" onclick="doScoreAdjust(-1)">➖ הפחת נקודות</button><button class="btn warning" onclick="resetStudentScore()">↺ אפס ניקוד</button></div>${hist.length?`<details class="score-history"><summary>יומן התאמות (${hist.length})</summary>${hist.map(h=>`<div class="score-history-row"><span><b>${Number(h.delta)>0?'+':''}${Number(h.delta)}</b> · ${esc(h.reason||h.action_type)}</span><small>${new Date(h.created_at).toLocaleString('he-IL')} · ${esc(h.admin_name||'מנהל')}</small></div>`).join('')}<button class="btn mini" onclick="undoLastScoreChange()">בטל שינוי ידני אחרון</button></details>`:''}</div>`}
