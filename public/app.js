...
function setManageSection(s){playSound('nav');adminManageSection=s;renderAdminContent();if(s==='rules')loadCampaignRulesEditor();if(s==='engagement')loadEngagementData();if(s==='score')setTimeout(()=>searchScoreStudents(true),0)}
...
function adminScoreManagement(){return `${adminPeriodsView()}<div class="panel"><h2>🎯 ניהול ניקוד תלמיד</h2><div class="small">חפש תלמיד ובצע התאמת ניקוד. כל פעולה דורשת סיסמת מנהל ונשמרת ביומן; הדיווחים המקוריים אינם נמחקים.</div><div class="score-search-wrap"><input id="scoreStudentSearch" class="input" placeholder="חיפוש לפי שם תלמיד..." oninput="clearTimeout(scoreSearchTimer);scoreSearchTimer=setTimeout(()=>searchScoreStudents(),250)"></div><div id="scoreStudentResults" class="score-student-results"></div><div id="scoreStudentCard"></div></div>`}
async function searchScoreStudents(initial=false){const el=$('scoreStudentResults');if(!el)return;const q=$('scoreStudentSearch')?.value||'';if(!initial&&q.trim().length<1){el.innerHTML='<div class="small">הקלד שם תלמיד כדי לחפש</div>';return}try{const rows=await api('/api/admin/score-management/students?q='+encodeURIComponent(q));el.innerHTML=rows.map(x=>`<button class="score-student-result" onclick="selectScoreStudent(${x.id},'${escAttr(x.name)}','${escAttr(x.group_name||'')}',${Number(x.score||0)})"><span><b>${esc(x.name)}</b><small>${esc(x.group_name||'ללא קבוצת לימודים')}</small></span><strong>${Number(x.score||0)} נק׳</strong></button>`).join('')||'<div class="small">לא נמצאו תלמידים</div>'}catch(e){el.innerHTML='<div class="small">'+esc(e.message)+'</div>'}}
async function selectScoreStudent(id,name,group,score){scoreSelectedStudent={id,name,group,score};await renderScoreStudentCard()}
async function renderScoreStudentCard(){const x=scoreSelectedStudent,el=$('scoreStudentCard');if(!x||!el)return;let hist=[];try{hist=await api('/api/admin/score-management/'+x.id+'/history')}catch{}el.innerHTML=`<div class="score-editor-card"><div class="score-editor-head"><div><h3>${esc(x.name)}</h3><div class="small">${esc(x.group||'ללא קבוצה')}</div></div><div class="score-current"><span>ניקוד נוכחי</span><b>${Number(x.score||0)}</b></div></div><label class="small">מספר נקודות</label><input id="scoreDelta" class="input" type="number" step="1" placeholder="למשל 10"><label class="small">סיבת השינוי (מומלץ)</label><input id="scoreReason" class="input" maxlength="300" placeholder="למשל: תיקון דיווח / בונוס מיוחד"><div class="score-actions"><button class="btn ok" onclick="doScoreAdjust(1)">➕ הוסף נקודות</button><button class="btn danger" onclick="doScoreAdjust(-1)">➖ הפחת נקודות</button><button class="btn warning" onclick="resetStudentScore()">↺ אפס ניקוד</button></div>${hist.length?`<details class="score-history"><summary>יומן התאמות (${hist.length})</summary>${hist.map(h=>`<div class="score-history-row"><span><b>${Number(h.delta)>0?'+':''}${Number(h.delta)}</b> · ${esc(h.reason||h.action_type)}</span><small>${new Date(h.created_at).toLocaleString('he-IL')} · ${esc(h.admin_name||'מנהל')}</small></div>`).join('')}<button class="btn mini" onclick="undoLastScoreChange()">בטל שינוי ידני אחרון</button></details>`:''}</div>`}
