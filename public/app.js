...
function adminScoreManagement(){
  ...
  const groupHtml=groups.map(g=>`<details class="admin-collapsible group-collapse score-group-collapse"><summary><span class="summary-icon">🏫</span><span class="grow"><b>${esc(g.name)}</b><small>${(g.members||[]).length} תלמידים</small></span><span class="summary-arrow" aria-hidden="true"></span></summary><div class="collapsible-body score-group-students">${(g.members||[]).map(m=>`<button class="score-student-result" onclick="selectScoreStudentById(${m.id},'${escAttr(m.name)}','${escAttr(g.name)}')"><span><b>${esc(m.name)}</b></span><span>ניהול ניקוד ›</span></button>`).join('')||'<div class="small empty-admin-list">אין תלמידים בקבוצה</div>'}</div></details>`).join('');
  const ungroupedHtml=ungrouped.length?`<details class="admin-collapsible group-collapse score-group-collapse"><summary><span class="summary-icon">👤</span><span class="grow"><b>ללא קבוצת לימודים</b><small>${ungrouped.length} תלמידים</small></span><span class="summary-arrow" aria-hidden="true"></span></summary><div class="collapsible-body score-group-students">${ungrouped.map(m=>`<button class="score-student-result" onclick="selectScoreStudentById(${m.id},'${escAttr(m.name)}','')"><span><b>${esc(m.name)}</b></span><span>ניהול ניקוד ›</span></button>`).join('')}</div></details>`:'';
  return `${adminPeriodsView()}<div class="panel"><h2>🎯 ניהול ניקוד תלמיד</h2><div class="small">בחר קבוצת לימודים ולאחר מכן תלמיד. כל שינוי דורש סיסמת מנהל ונשמר ביומן.</div><div class="score-search-wrap"><input id="scoreStudentSearch" class="input" placeholder="או חפש תלמיד לפי שם..." oninput="clearTimeout(scoreSearchTimer);scoreSearchTimer=setTimeout(()=>searchScoreStudents(),250)"></div><div id="scoreStudentResults" class="score-student-results"></div><div class="group-admin-list score-groups-list">${groupHtml}${ungroupedHtml}</div><div id="scoreStudentCard"></div></div>`
}
async function selectScoreStudentById(id,name,group){try{const rows=await api('/api/admin/score-management/students?q='+encodeURIComponent(name));const x=rows.find(r=>Number(r.id)===Number(id));if(!x)throw new Error('התלמיד לא נמצא');selectScoreStudent(x.id,x.name,group||x.group_name||'',Number(x.score||0));setTimeout(()=>$('scoreStudentCard')?.scrollIntoView({behavior:'smooth',block:'start'}),80)}catch(e){alert(e.message)}}
