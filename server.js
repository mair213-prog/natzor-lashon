  ...
  const {rows}=await pool.query(`
    WITH p AS (SELECT id FROM score_periods WHERE active=true LIMIT 1), student_scores AS (
      SELECT s.id,
        ...
        COALESCE((SELECT SUM(a.delta) FROM score_adjustments a WHERE a.student_id=s.id AND a.period_id=(SELECT id FROM p)),0)::int adjustment
      ...
      SELECT g.id,g.name,g.goal_target,COALESCE(SUM(ss.plus_count*3-ss.minus_count+FLOOR(ss.plus_count/20.0)*5+ss.adjustment),0)::int score
...
app.get('/api/groups/:id/students',auth,async(req,res)=>{
  ...
  const {rows}=await pool.query(`
    WITH p AS (SELECT id FROM score_periods WHERE active=true LIMIT 1), scores AS (
      ...
      (plus_count*3-minus_count+FLOOR(plus_count/20.0)*5+COALESCE((SELECT SUM(a.delta) FROM score_adjustments a WHERE a.student_id=scores.id AND a.period_id=(SELECT id FROM p)),0))::int score,
...
app.get('/api/public/results',async(req,res)=>{
  try{
    ...
    const {rows:students}=await pool.query(`WITH period AS (
      ...
      SELECT sc.*, (sc.p*3-sc.m+FLOOR(sc.p/20.0)*5+COALESCE((SELECT SUM(a.delta) FROM score_adjustments a WHERE a.student_id=sc.id AND a.period_id=$1),0))::int score,
      ...
      FROM ranked r
    ) SELECT r.id,r.name,r.score,r.participation_percent,r.clean_language_percent,r.leadership_score,r.eligible_leader,
      ...
      FROM scored r WHERE r.eligible_leader=true ORDER BY r.leadership_score DESC,r.score DESC,r.name LIMIT 20`,[pid]);
    const {rows:leaders}=await pool.query(`WITH scores AS (
      SELECT s.id,(COUNT(r.id) FILTER(WHERE r.report_type='plus')*3-COUNT(r.id) FILTER(WHERE r.report_type='minus')+FLOOR((COUNT(r.id) FILTER(WHERE r.report_type='plus'))/20.0)*5+COALESCE((SELECT SUM(a.delta) FROM score_adjustments a WHERE a.student_id=s.id AND a.period_id=$1),0))::int score
...
app.get('/api/admin/classes/:id/profile',auth,admin,async(req,res)=>{
  ...
  const adjQ=await pool.query(`SELECT COALESCE(SUM(a.delta) FILTER(WHERE (a.created_at AT TIME ZONE 'Asia/Jerusalem')::date=(now() AT TIME ZONE 'Asia/Jerusalem')::date),0)::int today_adjustment,COALESCE(SUM(a.delta) FILTER(WHERE a.created_at>=NOW()-INTERVAL '7 days'),0)::int week_adjustment FROM score_adjustments a JOIN group_students gs ON gs.student_id=a.student_id WHERE gs.group_id=$1 AND a.period_id=$2`,[groupId,pid]);
  const adj=adjQ.rows[0]||{},todayAdjustment=Number(adj.today_adjustment||0),weekAdjustment=Number(adj.week_adjustment||0);
  ...
  res.json({...g,score,max_possible:maxPossible,progress_percent:maxPossible?Math.max(0,Math.round(score/maxPossible*100)):0,days_elapsed:days,today:{plus:Number(x.plus_today||0),minus:Number(x.minus_today||0),adjustment:todayAdjustment,participation_percent:todayParticipation,balance:Number(x.plus_today||0)*3-Number(x.minus_today||0)+todayAdjustment},week:{plus:Number(x.plus_week||0),minus:Number(x.minus_week||0),adjustment:weekAdjustment,balance:Number(x.plus_week||0)*3-Number(x.minus_week||0)+weekAdjustment},all:{plus:Number(x.plus_all||0),minus:Number(x.minus_all||0),balance:score},students});
});

app.get('/api/admin/daily-summary',auth,admin,async(req,res)=>{
  const pid=await activePeriodId();
  const {rows}=await pool.query(`SELECT g.id,g.name,COUNT(DISTINCT gs.student_id)::int students,COUNT(r.id) FILTER(WHERE r.report_type='plus')::int plus,COUNT(r.id) FILTER(WHERE r.report_type='minus')::int minus,COALESCE(adj.adjustment,0)::int adjustment FROM groups g LEFT JOIN group_students gs ON gs.group_id=g.id LEFT JOIN reports r ON r.student_id=gs.student_id AND r.period_id=$1 AND (r.created_at AT TIME ZONE 'Asia/Jerusalem')::date=(now() AT TIME ZONE 'Asia/Jerusalem')::date LEFT JOIN LATERAL (SELECT SUM(a.delta)::int adjustment FROM score_adjustments a JOIN group_students ags ON ags.student_id=a.student_id WHERE ags.group_id=g.id AND a.period_id=$1 AND (a.created_at AT TIME ZONE 'Asia/Jerusalem')::date=(now() AT TIME ZONE 'Asia/Jerusalem')::date) adj ON true WHERE g.type='class' AND g.active=true GROUP BY g.id,g.name,adj.adjustment ORDER BY g.name`,[pid]);
  res.json(rows.map(x=>({...x,participation_percent:Number(x.students)?Math.round(Number(x.plus)/(Number(x.students)*2)*100):0,balance:Number(x.plus)*3-Number(x.minus)+Number(x.adjustment||0)})));
});
...
app.get('/api/admin/periods',auth,admin,async(req,res)=>{const {rows}=await pool.query(`SELECT p.*,COUNT(r.id)::int reports,COUNT(DISTINCT r.student_id)::int students,COALESCE(MAX(a.delta),0)::int manual_adjustments FROM score_periods p LEFT JOIN reports r ON r.period_id=p.id LEFT JOIN (SELECT period_id,SUM(delta) delta FROM score_adjustments GROUP BY period_id) a ON a.period_id=p.id GROUP BY p.id ORDER BY p.started_at DESC`);res.json(rows)});
app.post('/api/admin/periods/start',auth,admin,async(req,res)=>{
  ...
  const started=req.body.started_at?new Date(req.body.started_at):new Date();if(Number.isNaN(started.getTime())||started>Date.now()+60000)return res.status(400).json({error:'תאריך התחלה לא תקין'});
  const client=await pool.connect();try{await client.query('BEGIN');await client.query('UPDATE score_periods SET active=false,ended_at=COALESCE(ended_at,now()) WHERE active=true');const {rows}=await client.query('INSERT INTO score_periods(name,active,started_at) VALUES($1,true,$2) RETURNING *',[name,started]);await client.query('COMMIT');res.json(rows[0])}catch(e){await client.query('ROLLBACK');throw e}finally{client.release()}
});
app.put('/api/admin/periods/:id/name',auth,admin,async(req,res)=>{if(!validManagementPassword(req.body.management_password))return res.status(403).json({error:'סיסמת הניהול הראשית שגויה'});const name=String(req.body.name||'').trim();if(!name)return res.status(400).json({error:'יש להזין שם'});const {rows}=await pool.query('UPDATE score_periods SET name=$1 WHERE id=$2 RETURNING *',[name,Number(req.params.id)]);res.json(rows[0])});

async function studentCurrentScore(studentId,periodId){
  // Keep report aggregation and manual adjustments in separate queries. This avoids
  // coupling an aggregate over reports to a scalar subquery and makes the live
  // score endpoint deterministic even when a student has no reports yet.
  const [rq,aq]=await Promise.all([
    pool.query(`SELECT COUNT(*) FILTER(WHERE report_type='plus')::int plus_count, COUNT(*) FILTER(WHERE report_type='minus')::int minus_count FROM reports WHERE student_id=$1 AND period_id=$2`,[studentId,periodId]),
    pool.query(`SELECT COALESCE(SUM(delta),0)::int adjustment FROM score_adjustments WHERE student_id=$1 AND period_id=$2`,[studentId,periodId])
  ]);
...
app.post('/api/admin/score-management/:id/adjust',auth,admin,async(req,res)=>{if(!MANAGEMENT_PASSWORD)return res.status(500).json({error:'סיסמת ניהול ראשית עדיין לא הוגדרה בשרת'});if(!validManagementPassword(req.body.management_password))return res.status(403).json({error:'סיסמת הניהול הראשית שגויה'});const sid=Number(req.params.id),pid=await activePeriodId(),mode=String(req.body.mode||'adjust'),reason=String(req.body.reason||'').trim().slice(0,300);if(!sid)return res.status(400).json({error:'תלמיד לא תקין'});const before=await studentCurrentScore(sid,pid);let delta;if(mode==='reset')delta=-before;else{delta=Math.trunc(Number(req.body.delta));if(!Number.isFinite(delta)||delta===0)return res.status(400).json({error:'יש להזין מספר נקודות שונה מאפס'});}const {rows}=await pool.query(`INSERT INTO score_adjustments(student_id,period_id,delta,reason,action_type,admin_user_id) VALUES($1,$2,$3,$4,$5,$6) RETURNING *`,[sid,pid,delta,reason,mode==='reset'?'reset':'adjustment',req.user.id]);const after=await studentCurrentScore(sid,pid);res.json({...rows[0],before,after})});
app.post('/api/admin/score-management/:id/undo-last',auth,admin,async(req,res)=>{if(!validManagementPassword(req.body.management_password))return res.status(403).json({error:'סיסמת הניהול הראשית שגויה'});const sid=Number(req.params.id),pid=await activePeriodId();const {rows}=await pool.query('SELECT * FROM score_adjustments WHERE student_id=$1 AND period_id=$2 ORDER BY id DESC LIMIT 1',[sid,pid]);if(!rows[0])return res.status(404).json({error:'אין שינוי ידני לביטול'});const a=rows[0];await pool.query(`INSERT INTO score_adjustments(student_id,period_id,delta,reason,action_type,admin_user_id) VALUES($1,$2,$3,$4,'undo',$5)`,[sid,pid,-Number(a.delta),`ביטול שינוי #${a.id}`,req.user.id]);res.json({ok:true})});
...
async function ensureV2Schema(){
  ...
  await pool.query(`CREATE TABLE IF NOT EXISTS announcements (id SERIAL PRIMARY KEY,title TEXT NOT NULL,body TEXT NOT NULL,active BOOLEAN NOT NULL DEFAULT TRUE,expires_at TIMESTAMPTZ,created_at TIMESTAMPTZ NOT NULL DEFAULT NOW())`);
  ...
  await pool.query(`CREATE TABLE IF NOT EXISTS score_adjustments (id BIGSERIAL PRIMARY KEY,student_id INTEGER NOT NULL REFERENCES students(id) ON DELETE CASCADE,period_id INTEGER NOT NULL REFERENCES score_periods(id) ON DELETE CASCADE,delta INTEGER NOT NULL,reason TEXT NOT NULL DEFAULT '',action_type TEXT NOT NULL DEFAULT 'adjustment',admin_user_id INTEGER REFERENCES users(id) ON DELETE SET NULL,created_at TIMESTAMPTZ NOT NULL DEFAULT NOW())`);
  await pool.query(`CREATE INDEX IF NOT EXISTS idx_score_adjustments_student_period ON score_adjustments(student_id,period_id)`);
}
...
app.get('/api/home',auth,async(req,res)=>{
 ...
 const [sum,top,groups,ann,ch]=await Promise.all([
  ...
  pool.query(`WITH period AS (SELECT GREATEST(1,((now() AT TIME ZONE 'Asia/Jerusalem')::date-(started_at AT TIME ZONE 'Asia/Jerusalem')::date)+1)::int days FROM score_periods WHERE id=$1), sc AS (SELECT s.id,s.name,COUNT(r.id) FILTER(WHERE r.report_type='plus')::int p,COUNT(r.id) FILTER(WHERE r.report_type='minus')::int m FROM students s LEFT JOIN reports r ON r.student_id=s.id AND r.period_id=$1 WHERE s.active=true GROUP BY s.id,s.name), x AS (SELECT sc.*,(sc.p*3-sc.m+FLOOR(sc.p/20.0)*5+COALESCE((SELECT SUM(a.delta) FROM score_adjustments a WHERE a.student_id=sc.id AND a.period_id=$1),0))::int score,LEAST(100,ROUND(sc.p::numeric/NULLIF(period.days*2,0)*100))::int participation_percent,GREATEST(0,LEAST(100,ROUND(100-(sc.m::numeric/period.days)*20)))::int clean_language_percent FROM sc CROSS JOIN period), scored AS (SELECT x.*,ROUND(clean_language_percent*0.70+participation_percent*0.30)::int leadership_score,(participation_percent>=50) eligible_leader FROM x) SELECT id,name,score,participation_percent,clean_language_percent,leadership_score,eligible_leader FROM scored WHERE eligible_leader=true ORDER BY leadership_score DESC,score DESC,name LIMIT 5`,[pid]),
  pool.query(`WITH ss AS (SELECT s.id,(${scoreSql()}+COALESCE((SELECT SUM(a.delta) FROM score_adjustments a WHERE a.student_id=s.id AND a.period_id=$1),0))::int score FROM students s LEFT JOIN reports r ON r.student_id=s.id AND r.period_id=$1 GROUP BY s.id) SELECT g.id,g.name,COALESCE(SUM(ss.score),0)::int score FROM groups g LEFT JOIN group_students gs ON gs.group_id=g.id LEFT JOIN ss ON ss.id=gs.student_id WHERE g.type='class' AND g.active=true GROUP BY g.id,g.name ORDER BY score DESC LIMIT 5`,[pid]),
  pool.query(`SELECT * FROM announcements WHERE active=true AND (expires_at IS NULL OR expires_at>NOW()) ORDER BY created_at DESC LIMIT 3`),
  pool.query(`SELECT c.*,COALESCE(cs.current_score,0)::int current_score,LEAST(100,GREATEST(0,ROUND((COALESCE(cs.current_score,0)::numeric/NULLIF(c.target_points,0))*100)))::int progress FROM challenges c LEFT JOIN LATERAL (SELECT COALESCE(SUM(x.student_score),0)::int current_score FROM (SELECT r.student_id,(COUNT(r.id) FILTER(WHERE r.report_type='plus')*3-COUNT(r.id) FILTER(WHERE r.report_type='minus')+FLOOR((COUNT(r.id) FILTER(WHERE r.report_type='plus'))/20.0)*5)::int student_score FROM reports r WHERE r.period_id=$1 AND r.created_at>=c.start_at AND (c.end_at IS NULL OR r.created_at<=c.end_at) GROUP BY r.student_id UNION ALL SELECT a.student_id,SUM(a.delta)::int student_score FROM score_adjustments a WHERE a.period_id=$1 AND a.created_at>=c.start_at AND (c.end_at IS NULL OR a.created_at<=c.end_at) GROUP BY a.student_id) x) cs ON true WHERE c.active=true AND (c.end_at IS NULL OR c.end_at>NOW()) ORDER BY c.created_at DESC LIMIT 3`,[pid])
 ]);
...
app.get('/api/students/:id/profile',auth,async(req,res)=>{
 ...
 if(!rows[0])return res.status(404).json({error:'תלמיד לא נמצא'}); const st=rows[0]; const aq=await pool.query('SELECT COALESCE(SUM(delta),0)::int adjustment FROM score_adjustments WHERE student_id=$1 AND period_id=$2',[id,pid]); st.adjustment=Number(aq.rows[0]?.adjustment||0); st.score=Number(st.score||0)+st.adjustment;
 const hist=await pool.query(`WITH days AS (SELECT generate_series((now() AT TIME ZONE 'Asia/Jerusalem')::date-13,(now() AT TIME ZONE 'Asia/Jerusalem')::date,'1 day'::interval)::date AS report_day), daily AS (SELECT (created_at AT TIME ZONE 'Asia/Jerusalem')::date AS report_day,SUM(CASE WHEN report_type='plus' THEN 3 ELSE -1 END)::int AS points FROM reports WHERE student_id=$1 AND period_id=$2 AND (created_at AT TIME ZONE 'Asia/Jerusalem')::date>=(now() AT TIME ZONE 'Asia/Jerusalem')::date-13 GROUP BY 1), adjustments AS (SELECT (created_at AT TIME ZONE 'Asia/Jerusalem')::date AS report_day,SUM(delta)::int AS adjustment FROM score_adjustments WHERE student_id=$1 AND period_id=$2 AND (created_at AT TIME ZONE 'Asia/Jerusalem')::date>=(now() AT TIME ZONE 'Asia/Jerusalem')::date-13 GROUP BY 1) SELECT to_char(days.report_day,'YYYY-MM-DD') AS day,to_char(days.report_day,'DD.MM') AS label,(COALESCE(daily.points,0)+COALESCE(adjustments.adjustment,0))::int AS points,COALESCE(adjustments.adjustment,0)::int AS adjustment FROM days LEFT JOIN daily USING(report_day) LEFT JOIN adjustments USING(report_day) ORDER BY days.report_day`,[id,pid]);
 ...
 const todayAdj=await pool.query(`SELECT COALESCE(SUM(delta),0)::int adjustment FROM score_adjustments WHERE student_id=$1 AND period_id=$2 AND (created_at AT TIME ZONE 'Asia/Jerusalem')::date=(now() AT TIME ZONE 'Asia/Jerusalem')::date`,[id,pid]);
 ...
 res.json({...st,streak:streak.rows[0]?.streak||0,badges,history:hist.rows,today:{plus:Number(today.rows[0]?.plus||0),minus:Number(today.rows[0]?.minus||0),adjustment:Number(todayAdj.rows[0]?.adjustment||0),balance:Number(today.rows[0]?.plus||0)*3-Number(today.rows[0]?.minus||0)+Number(todayAdj.rows[0]?.adjustment||0)}});
});
...
app.get('/api/announcements',auth,async(req,res)=>{const {rows}=await pool.query(`SELECT * FROM announcements WHERE active=true AND (expires_at IS NULL OR expires_at>NOW()) ORDER BY created_at DESC LIMIT 20`);res.json(rows)});
app.get('/api/admin/announcements',auth,admin,async(req,res)=>{const {rows}=await pool.query(`SELECT * FROM announcements ORDER BY created_at DESC LIMIT 100`);res.json(rows)});
app.post('/api/admin/announcements',auth,admin,async(req,res)=>{const title=String(req.body.title||'').trim(),body=String(req.body.body||'').trim();if(!title||!body)return res.status(400).json({error:'יש למלא כותרת ותוכן'});const expires=req.body.expires_at||null;const {rows}=await pool.query(`INSERT INTO announcements(title,body,expires_at) VALUES($1,$2,$3) RETURNING *`,[title,body,expires]);res.json(rows[0])});
app.put('/api/admin/announcements/:id',auth,admin,async(req,res)=>{const id=Number(req.params.id),title=String(req.body.title||'').trim(),body=String(req.body.body||'').trim();if(!title||!body)return res.status(400).json({error:'יש למלא כותרת ותוכן'});const {rows}=await pool.query(`UPDATE announcements SET title=$1,body=$2,expires_at=$3,active=COALESCE($4,active) WHERE id=$5 RETURNING *`,[title,body,req.body.expires_at||null,typeof req.body.active==='boolean'?req.body.active:null,id]);if(!rows[0])return res.status(404).json({error:'הודעה לא נמצאה'});res.json(rows[0])});
app.delete('/api/admin/announcements/:id',auth,admin,async(req,res)=>{await pool.query('DELETE FROM announcements WHERE id=$1',[Number(req.params.id)]);res.json({ok:true})});

app.get('/api/challenges',auth,async(req,res)=>{const {rows}=await pool.query(`SELECT * FROM challenges WHERE active=true AND (end_at IS NULL OR end_at>NOW()) ORDER BY created_at DESC`);res.json(rows)});
app.get('/api/admin/challenges',auth,admin,async(req,res)=>{const pid=await activePeriodId();const {rows}=await pool.query(`SELECT c.*,COALESCE(cs.current_score,0)::int current_score,LEAST(100,GREATEST(0,ROUND((COALESCE(cs.current_score,0)::numeric/NULLIF(c.target_points,0))*100)))::int progress FROM challenges c LEFT JOIN LATERAL (SELECT COALESCE(SUM(x.student_score),0)::int current_score FROM (SELECT r.student_id,(COUNT(r.id) FILTER(WHERE r.report_type='plus')*3-COUNT(r.id) FILTER(WHERE r.report_type='minus')+FLOOR((COUNT(r.id) FILTER(WHERE r.report_type='plus'))/20.0)*5)::int student_score FROM reports r WHERE r.period_id=$1 AND r.created_at>=c.start_at AND (c.end_at IS NULL OR r.created_at<=c.end_at) GROUP BY r.student_id UNION ALL SELECT a.student_id,SUM(a.delta)::int student_score FROM score_adjustments a WHERE a.period_id=$1 AND a.created_at>=c.start_at AND (c.end_at IS NULL OR a.created_at<=c.end_at) GROUP BY a.student_id) x) cs ON true ORDER BY c.created_at DESC LIMIT 100`,[pid]);res.json(rows)});
