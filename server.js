...
async function studentCurrentScore(studentId,periodId){const q=await pool.query(`SELECT (COUNT(r.id) FILTER(WHERE r.report_type='plus')*3-COUNT(r.id) FILTER(WHERE r.report_type='minus')+FLOOR((COUNT(r.id) FILTER(WHERE r.report_type='plus'))/20.0)*5+COALESCE((SELECT SUM(a.delta) FROM score_adjustments a WHERE a.student_id=$1 AND a.period_id=$2),0))::int score FROM reports r WHERE r.student_id=$1 AND r.period_id=$2`,[studentId,periodId]);return Number(q.rows[0]?.score||0)}
